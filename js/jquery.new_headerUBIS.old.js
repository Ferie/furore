...
(function($) {
	...
	var methods = {
		init: function(options){
			...
			st.$header.find('.sidebar-switch a').click(function(){utils._toggleSidebar2();});
	...
	var utils = {
		...
		_toggleSidebar2: function(){
			...
			var speed = Math.abs(widthS-oldWidthS)*st.pixelSpeed;
			if (!st.$sidebar.hasClass("opened")){
				//CLOSE
			...
			if (isBreakpoint(st.breakpoint)) {
				st.$sidebar.find(".sidebar-scroller, .sidebar-container").each(function(i,e){
				...
				st.$container.css({"width": widthC+"px"}).toggleClass("opened");
				st.$container.find(".mainContainer-wrapper").css({width: widthC+"px"});
				st.$container.trigger(st.animationEvent);
			} else {
				st.$sidebar.find(".sidebar-scroller, .sidebar-container").animate({"width": widthS+'px'},
						{
						complete: function(){
							$(this).hasClass("sidebar-container")?$(this).height($(window).height()*2):$(this).height($(window).height());
							} 
				...
				st.$container.animate({
					...
					speed, 
				"linear");
				st.$container.find(".mainContainer-wrapper").animate({
					width: widthC+"px"},
					speed, 
				"linear");
		...
		_toggleSidebar: function(action){
			...
			switch (action){
				case "open":
					st.$container.trigger(st.openEvent);
					st.$header.find('.sidebar-switch a').addClass("opened");
					st.$sidebar.find(".sidebar-scroller, .sidebar-container").animate({"width": utils._getSidebarWidth()+'px'}, {
						"queue": false, 
						complete: function(){
							$(this).hasClass("sidebar-container")?$(this).height($(window).height()*2):$(this).height($(window).height());
						}}, 
				...
				case "close":
					st.$container.trigger(st.closeEvent);
					st.$header.find('.sidebar-switch a').removeClass("opened");
					st.$sidebar.find(".sidebar-scroller").animate({"width": utils._getSidebarWidth()+'px'}, {
						"queue": false, 
						complete: function(){
						}}, 
						utils._getSidebarWidth()*st.pixelSpeed, 
		...
		_setPositionContainer: function(){
			...
			st.$sidebar.css({top: topC+"px"});
			st.$search.animate({
//				left: leftC+"px", 
				width: widthC+"px"}, {"queue": false}, "linear");
			st.$container.animate({
//				left: leftC+"px", 
				width: widthC+"px"}, {"queue": false}, "linear");
			st.$container.find(".mainContainer-wrapper").animate({
...
//				}, {"queue": false}, "linear");
		...
		_setPositionContainer2: function(){
			var widthS = utils._getSidebarWidth();
			var widthC =  $(window).outerWidth(true)-widthS;
			st.$search.css({width: widthC+"px"});
		...
		_setElementSizes: function(container){
			$(container).find("[data-ucg-height]").each(function(index, element){
				...
				$(element).data("old-style", $(element).attr("style"));
				$(element).data("old-height", $(element).attr("height"));
				$(element).data("old-width", $(element).attr("width"));
		...
		_unsetElementSizes: function(container){
			$(container).find("[data-ucg-height]").each(function(index, element){
				if(typeof $(element).data("old-style")!= "undefined"){
					$(element).attr("style", $(element).data("old-style"));
				} else {
					$(element).removeAttr("style");
				}
				if(typeof $(element).data("old-height")!= "undefined"){
					$(element).attr("height", $(element).data("old-height"));					
				} else {
					$(element).removeAttr("height");					
				}
				if(typeof $(element).data("old-width")!= "undefined"){
					$(element).attr("width", $(element).data("old-width"));					
				} else {
